...
public class Main {
	...
	static int setUpWall(int rest) {
		if(rest ==0) {
			...
			for(int i =0; i <N; ++i)
				System.arraycopy(map[i], 0, virtualMap[i], 0, M);
			spreadVirus(virtualMap);
			return getSafeArea(virtualMap);
		}
		int ret =0;
	...
	static void spreadVirus(int[][] virtualMap) {
		...
		while(!queue.isEmpty()) {
			...
			for(int dir =0; dir <4; ++dir) {
				...
				if(inRange(nextY, nextX) && virtualMap[nextY][nextX] ==0) {
					virtualMap[nextY][nextX] =2;
	...
	static int getSafeArea(int[][] virtualMap) {
		int ret =0;
